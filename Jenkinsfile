pipeline {
    ...
    stages {
        ...
        stage('Deploy Report') {
       steps {
           sh """curl -s -X POST http://host.docker.internal:8000/api/v1/pipeline/report \
               -H 'Content-Type: application/json' \
               -d '{"branch": "${env.BRANCH_NAME}", "build_number": "${env.BUILD_NUMBER}", "status": "SUCCESS", "duration_ms": ${currentBuild.duration}}'"""
           echo 'Report sent to backend'

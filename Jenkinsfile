    ...
    post {
        always {
            script {
                ...
                def reportData = '{"branch":"' + env.BRANCH_NAME + '","build_number":"' + env.BUILD_NUMBER + '","status":"' + buildStatus + '","duration_ms":' + currentBuild.duration + ',"deployed_url":"N/A"}'
                sh "curl -s -X POST http://host.docker.internal:8000/api/v1/pipeline/report -H 'Content-Type: application/json' -d '" + reportData + "' || true"
                echo 'Report sent: ' + buildStatus
            }
        }
    }
}
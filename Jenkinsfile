    ...
    stages {
        ...
        stage('Docker Build & Push') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-credentials', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
                    sh 'docker build --no-cache -t ${DOCKER_USER}/${DOCKER_IMAGE}:${BUILD_NUMBER} .'
        ...
        stage('Security Scan') {
            steps {
                ...
                sh 'docker inspect ${DOCKER_IMAGE}:${BUILD_NUMBER} --format "Image: {{.Id}} Size: {{.Size}}" || true'
                sh 'docker history ${DOCKER_IMAGE}:${BUILD_NUMBER} --no-trunc || true'
        ...
        stage('Terraform Deploy') {
            steps {
                withCredentials([
                    ...
                    sh 'cd terraform && terraform apply -auto-approve -input=false -var="docker_image=${DOCKER_USER}/${DOCKER_IMAGE}:${BUILD_NUMBER}"'
